...
그런데 요번엔 꼭 저렇게 풀지 않고 그냥 단순하게 풀어보고 싶어졌다. 내 생각은 이렇다.
...
그런데 틀렸다고 뜬다... 틀린 케이스를 못찾겠어서 질문글을 올리고 기다리는중
...
다른 코드들을 훑어 봤는데 죄다 DFS재귀 혹은 BFS여서 다른 방법으로 해결하고 싶었는데 역시 쉽게 되진 않는것 같다. 어떤 케이스에서 틀린건지를 알수가 없어서 너무 답답하다.