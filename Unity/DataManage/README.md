...
생각을 해보니 7번 과정은 여러개의 .json을 읽어서 하나로 합치고 런타임에 LocalGameData 클래스를 채우고 사용하는 것이였는데, LocalGameData를 런타임에 채울 필요가 있을까 라는 생각이 들었고, 코드와 과정을 좀 수정했다.

`Tools - Convert GameData`를 하면 `Data_Game.json`을 생성하기는 하지만 이 json 파일은 LocalGameData를 에디트 타임에 이미 생성해 놓고 클래스를 json으로 만든 파일이다.

즉 에디트 타임에 이미 LocalGameData를 구성해놓고 json으로 만들어 놓은 후 런타임에 파일만 json 파일만 읽어서 Deserialize만 해서 사용하면 훨씬 더 효율적인것 같다.
