...
* Add : 두 자료구조의 요소를 추가할경우 속도 차이가 나는건 아마 내부 구조상의 문제인듯 하다 List는 단순 배열인반면 Dictionary는 해싱함수에 따른 인덱스와 매칭되는 값을 저장하는 장소가 더있고, 크기가 커져 재할당시에 List보다 복잡한 연산이 필요할거라 생각하기 때문이다.

* Remove : 삭제에서는 Dictionary가 당연 우세하다 List는 순차탐색을 하는 반면 Dictionary는 연결되어 있는 노드를 끊어주기만 하면 되기때문에...라고 생각했는데 다시 생각해보니 결국엔 Remove 함수도 탐색해서 삭제하는것 아닌가? 속도차이는 List의 경우 삭제를 할 시 배열을 재구성하기 때문이고 Dictionary는 그럴 필요가 없기 때문이라고 생각한다.

* Search : 사실 탐색은 Dictionary의 속도가 훨씬 우세할줄 알았는데 예상과는 달라서 좀 당황했다. 그래도 Dictionary는 어느것을 찾든 평균적인 속도를 보여준 반면 List는 확실히 차이가 컸다.

내가 잘못 실험한 부분도 있을것 같고, Dictionary의 경우 크기가 커져서 재할당을 할시 혹은 충돌(Add 때만) 때문에 성능적인 이슈가 있는것 같기도 하다. 하지만 크게 의미없는 시간은 아니였던것 같다.
